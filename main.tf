...
resource "azurerm_network_interface_security_group_association" "example" {
  count                       = length(azurerm_network_interface.my_terraform_nic)
...
resource "azurerm_linux_virtual_machine" "my_terraform_vm" {
  count                 = 2  # Set the desired number of VMs
  ...
  admin_ssh_key {
    ...
    public_key = file("~/.ssh/id_rsa.pub")   # jsondecode(azapi_resource_action.ssh_public_key_gen.output).publicKey
  }
